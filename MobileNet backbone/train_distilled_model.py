...
def train(mob_encoder, mob_sencoder, mob_sdecoder, block, input_img):
    enc_feats = []
    senc_feats = []
    skip_feats = []
    ...
    loss_distill = mse(tf.linalg.einsum('bijd,cd->bijc', senc_feats[-1]-m2, mob_sencoder.eigenbases[block]), enc_feats[-1]-m1)

    sdec_feats = []    
    for l in reversed(range(block+1)):
        x = mob_sdecoder(l, x, skip=skip_feats[l])
        sdec_feats.append(x)
...
''' 
The MobileNet encoder is splitted into a series of blocks (enc0, enc1, enc2, enc3) from the input to the bottleneck.
...
mob_sdecoder = MobilenetSDecoder()
...
if args.pair != 0:
    for i in range(0, args.pair):
        ckpt = tf.train.Checkpoint(senc=mob_sencoder.btnecks[i], sdec=mob_sdecoder.btnecks[i])
        try:
...
for block in blocks:
    ...
    ckpt_aec = tf.train.Checkpoint(senc=mob_sencoder.btnecks[block], sdec=mob_sdecoder.btnecks[block])
    ...
    # warm up
    train(mob_encoder, mob_sencoder, mob_sdecoder, block, tf.random.normal([1, 256, 256, 3]))
    weights = mob_sencoder.btnecks[block].trainable_weights + mob_sdecoder.btnecks[block].trainable_weights 

    for epoch in range(args.epochs[block]):
        for i, imgs in enumerate(ds):
            with tf.GradientTape() as tape:
                loss = train(mob_encoder, mob_sencoder, mob_sdecoder, block, imgs) 
                _loss = sum(loss)
    ...
    if block == 3:
        mob_sencoder.save_weights(f"{os.path.join(args.saveto, 'sencoder')}")
        mob_sdecoder.save_weights(f"{os.path.join(args.saveto, 'sdecoder')}")
